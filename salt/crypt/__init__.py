# -*- coding: utf-8 -*-
'''
Abstract base class for pluggable auth facotry
'''
...
class CryptoFactory(object):  # TODO better name?!
    @staticmethod
    def factory(opts):
        # Begin selection of crypto library
        if opts['crypto_lib'] == 'pycrypto':
            import salt.crypt.pycrypto
            return salt.crypt.pycrypto.Crypt(opts)
        elif opts['crypto_lib'] == 'nacl' or opts['crypto_lib'] == 'libnacl':
            import salt.crypt.libnacl
            return salt.crypt.libnacl.Crypt(opts)
...
def dropfile(cachedir, user=None):
    ...
    mask = os.umask(191)
    try:
        log.info('Rotating master publisher session key')
...
def sign_message(opts, privkey_path, message):
    ...
    '''
    return CryptoFactory.factory(opts).sign_message(privkey_path, message)


def verify_signature(opts, pubkey_path, message, signature):
    '''
...
def gen_signature(opts, priv_path, pub_path, sign_path):
    ...
    mpub_sig = sign_message(opts, priv_path, mpub_64)
...
def private_encrypt(opts, key, message):
    #TODO Modify docs to be library-agnostic
    '''
    ...
    '''
    return CryptoFactory.factory(opts).private_encrypt(key, message)


def public_decrypt(opts, pub, message):
    #TODO Modify docs to be library-agnostic
    '''
    Verify a signature
...
class AsyncAuth(object):
    ...
    def gen_token(self, clear_tok):
        ...
        :rtype: str
        '''
        return private_encrypt(self.opts, self.get_keys(), clear_tok)
    ...
    def verify_pubkey_sig(self, message, sig):
        ...
        if self.opts['master_sign_key_name']:
            ...
            if os.path.isfile(path):
                res = verify_signature(self.opts,
                                       path,
                                       message,
